...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	return float64(t.distance()) / t.Duration.Hours()
}
...
/*  Calories возввращает количество потраченных килокалория при беге.
	...
	(множитель средней скорости бега * средняя_скорость_в_км/ч + коэффициент изменения средней скорости) * вес_спортсмена_в_кг / м_в_км * время_тренировки_в_часах * мин_в_часе
*/
...
/*
	 Calories возвращает количество потраченных килокалорий при ходьбе.
		Это переопределенный метод Calories() из Training
		(коэффициент для веса * вес_спортсмена_в_кг + (средняя_скорость_в_метрах_в_секунду^2 / рост_в_метрах) * коэффициент для роста * вес_спортсмена_в_кг) * время_тренировки_в_часах * мин_в_ч
		1 м/с равен 3,6 км/ч
*/
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	caclSpeed := math.Pow(w.meanSpeed()*KmInHtoMH, 2)
	return ((CaloriesWeightMultiplier*w.Training.Weight + caclSpeed/w.Height) * CaloriesSpeedHeightMultiplier * w.Training.Weight) * w.Training.Duration.Hours() * MinInHours
}
